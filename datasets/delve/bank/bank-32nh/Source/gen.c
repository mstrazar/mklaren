...
void setup(void)
{
  ...
  temperature = 1.0/(1.0+exp(rand_gaus(rs2)));
  for (i=0; i<no_areas; i++) {
    area[i].centre.x = 2.0*erand48(rs2)-1.0;
    area[i].centre.y = 2.0*erand48(rs2)-1.0;
    area[i].cov.sigma_x = rand_exp(rs2);
    area[i].cov.sigma_y = rand_exp(rs2);
    area[i].cov.rho = erand48(rs2)-0.5;
    area[i].pop = 3.1*rand_exp(rs2);
  }
  mxql = (int) (5.0 + 5.0*erand48(rs2));
  for (i=0; i<no_banks; i++) {                    /* Initialize the location */
    bank[i].loc.x = 2.0*erand48(rs2)-1.0;          /* and sizes of the banks */
    bank[i].loc.y = 2.0*erand48(rs2)-1.0;
    bank[i].max_q_lengths = mxql;
    bank[i].call = (int) (2.0 + (mxql-2.0)*erand48(rs2));
    bank[i].queue[0].efficiency = 0.5+1.5*erand48(rs2);
