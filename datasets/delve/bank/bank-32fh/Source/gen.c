...
void setup(void)
{
  ...
  temperature = 1.0/(1.0+exp(0.2*rand_gaus(rs2)));
  area[0].centre.x =  0.5 + 0.2*erand48(rs2);
  area[0].centre.y =  0.7 + 0.3*erand48(rs2);
  area[1].centre.x = -0.2 - 0.1*erand48(rs2);
  area[1].centre.y =  0.8 + 0.1*erand48(rs2);
  area[2].centre.x = -0.3 * 0.2*erand48(rs2);
  area[2].centre.y = -0.4 * 0.1*erand48(rs2);
  for (i=0; i<no_areas; i++) {
    area[i].cov.sigma_x = 0.5 + 0.1*rand_exp(rs2);
    area[i].cov.sigma_y = 0.5 + 0.1*rand_exp(rs2);
    area[i].cov.rho = 0.2*erand48(rs2)-0.1;
    area[i].pop = 3.1*rand_exp(rs2);
  }
  mxql = (int) (5.0 + 4.0*erand48(rs2));
  bank[0].loc.x =  0.1 + 0.1*erand48(rs2);
  bank[0].loc.y =  0.1 + 0.2*erand48(rs2);
  bank[1].loc.x = -0.2 + 0.3*erand48(rs2);  
  bank[1].loc.y = -0.5 + 0.4*erand48(rs2);
  bank[2].loc.x = -0.7 + 0.1*erand48(rs2);  
  bank[2].loc.y =  0.1 + 0.1*erand48(rs2);
  for (i=0; i<no_banks; i++) {                       /* Initialize the banks */
    bank[i].max_q_lengths = mxql;
    bank[i].call = (int) (2.0 + (mxql-2.0)*erand48(rs2));
    bank[i].queue[0].efficiency = 0.9+0.2*erand48(rs2);
...
double do_sim(void)
{
  area[0].pop = area[0].pop * erand48(rs1);
  area[1].pop = area[1].pop * (erand48(rs1) + 1.0)/2.0;
  area[2].pop = area[2].pop * erand48(rs1);
  area[0].centre.x = (area[0].centre.x + erand48(rs1) - 0.5)/1.5;
  area[0].centre.y = (area[0].centre.y + erand48(rs1) - 0.5)/1.5;
  area[1].centre.y = (area[1].centre.y + erand48(rs1) - 0.5)/1.5;
  area[2].centre.x = (area[2].centre.x + erand48(rs1) - 0.5)/1.5;
  bank[0].loc.x = (bank[0].loc.x + erand48(rs1) - 0.5)/1.5;
  bank[0].loc.y = (bank[0].loc.y + erand48(rs1) - 0.5)/1.5;
  bank[1].loc.x += bank[1].loc.y;
  bank[2].loc.y += bank[2].loc.x;
  bank[0].queue[0].efficiency = 0.5+1.5*erand48(rs1);
  bank[1].queue[0].efficiency = 0.5+1.5*erand48(rs1);
  bank[2].queue[0].efficiency = 0.5+1.5*erand48(rs1);
  if (erand48(rs1) > 0.5) bank[0].max_q_lengths += 3;
  return sim();
}
...
void gen_data(int n)
{
  ...
  for (c=0; c<n; c++) {
    setup();
    for (i=0; i<no_areas; i++) {
      printf("%10.6f ", area[i].centre.x);
      printf("%10.6f ", area[i].centre.y);
      printf("%10.6f ", area[i].cov.sigma_x);
      printf("%10.6f ", area[i].cov.sigma_y);
      printf("%10.6f ", area[i].cov.rho);
      printf("%10.6f ", area[i].pop);
    }
    printf("%10.6f ", temperature);
    for (i=0; i<no_banks; i++) {
      printf("%10.6f ", bank[i].loc.x);
      printf("%10.6f ", bank[i].loc.y);
      printf("%4d ", bank[i].call);  
      printf("%10.6f ", bank[i].queue[0].efficiency);
    }
